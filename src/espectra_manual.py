...
*** NEW: 'Total' plots are summed, RENORMALIZED, then scaled ***
- The "Total Computed" and "Total Vibronic" spectra are now
  calculated by summing the individually *scaled* spectra,
  then this sum is *re-normalized to 1.0*, and finally, the
  manual scaling factor for the "Total" plot is applied.

*** MODIFIED (User Request): Summation Logic ***
- If the user selects 'y' to scale *individual* 'raw' files,
  the 'Total Computed' sum is created from (normalized * scaled) spectra.
- If the user selects 'n' (or default), the 'Total Computed' sum
  is created from the *raw, un-normalized* convoluted spectra.
- The same logic is applied to the 'vib' files for the 'Total Vibronic' sum.
"""
...
def get_cli_args():
    """
    Parses command-line arguments for broadening function type, FWHM,
    and manual energy shifts.
    """
    parser = argparse.ArgumentParser(description="Generate and plot absorption spectra.")
    ...
    parser.add_argument(
        ...
        help="Manual energy shift (eV) for all 'raw' (computed) spectra."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None, # Default to None to check if it was provided
        help="Manual energy shift (eV) for all 'vib' (vibronic) spectra."
    )
    ...
    return args.func, args.fwhm, args.shift_computed, args.shift_vibronic
...
def get_manual_scaling(raw_files, vib_files, generate_total_computed, generate_total_vibronic):
    ...
    Returns:
    - scaling_factors (dict): Maps filename to scaling factor.
    - scale_choice_raw (bool): True if user opted to scale individual raw files.
    - scale_choice_vib (bool): True if user opted to scale individual vib files.
    """
    scaling_factors = {}
    
    # --- MODIFICATION: Track the user's 'y'/'n' choice ---
    scale_choice_raw = False
    scale_choice_vib = False
    ...
    if raw_files:
        ...
        if choice_raw == 'y':
            # --- MODIFICATION: Record the choice ---
            scale_choice_raw = True
    ...
    if generate_total_computed:
        try:
            choice_total_raw = input("\nDo you want to manually scale the 'Total Computed' spectrum (after its renormalization)? [y/N]: ").strip().lower()
        except EOFError:
            choice_total_raw = 'n'
    ...
    if generate_total_vibronic:
        try:
            choice_total_vib = input("\nDo you want to manually scale the 'Total Vibronic' spectrum (after its renormalization)? [y/N]: ").strip().lower()
        except EOFError:
            choice_total_vib = 'n'
    ...
    # --- MODIFICATION: Return the scaling choices ---
    return scaling_factors, scale_choice_raw, scale_choice_vib
...
# --- NEW: Helper function to finalize and save plots ---
...
def main():      
    ...
    func_type, fwhm, manual_shift_eV_comp, manual_shift_eV_vib = get_cli_args()
    
    if manual_shift_eV_comp is None:
        try:
            shift_input = input("\nEnter manual shift (eV) for computed ('raw') spectra [Default: 0.0]: ").strip()
            if shift_input:
                manual_shift_eV_comp = float(shift_input)
            else:
                manual_shift_eV_comp = 0.0
        except (ValueError, EOFError):
            print("  > Invalid input or non-interactive mode. Using default 0.0 eV shift for 'raw'.")
            manual_shift_eV_comp = 0.0

    if manual_shift_eV_vib is None:
        try:
            shift_input = input("\nEnter manual shift (eV) for vibronic ('vib') spectra [Default: 0.0]: ").strip()
            if shift_input:
                manual_shift_eV_vib = float(shift_input)
            else:
                manual_shift_eV_vib = 0.0
        except (ValueError, EOFError):
            print("  > Invalid input or non-interactive mode. Using default 0.0 eV shift for 'vib'.")
            manual_shift_eV_vib = 0.0
    
    print("\nScript settings:")
    print(f"  Broadening function: {func_type}")
    print(f"  FWHM: {fwhm}")
    print(f"  Manual Shift (Computed): {manual_shift_eV_comp:+.3f} eV (applied to all 'raw' spectra)")
    print(f"  Manual Shift (Vibronic): {manual_shift_eV_vib:+.3f} eV (applied to all 'vib' spectra)")
    ...
    # --- NEW: Auto-generate 'Total' plots if more than one file ---
    ...
    # --- MODIFICATION: Get the scaling choices as well ---
    scaling_factors, scale_choice_raw, scale_choice_vib = get_manual_scaling(
        raw_files, vib_files, generate_total_computed, generate_total_vibronic
    )
    
    # --- MODIFICATION: Print summation logic ---
    if generate_total_computed:
        if scale_choice_raw:
            print("  > Summing 'Total Computed' from: (Normalized * Scaled) spectra.")
        else:
            print("  > Summing 'Total Computed' from: Raw (un-normalized) spectra.")
    if generate_total_vibronic:
        if scale_choice_vib:
            print("  > Summing 'Total Vibronic' from: (Normalized * Scaled) spectra.")
        else:
            print("  > Summing 'Total Vibronic' from: Raw (un-normalized) spectra.")
    ...
    # --- NEW: Create lists of all axes to plot experimental data on ---
    ...
    exp_plot_data = [] # To store data for individual plots
    for filepath in exp_files:
        filename = os.path.basename(filepath)
        ...
        try:
            ...
            # --- NEW: Styling for experimental data ---
            style = '--'
            width = 2.0
            color = 'black'
            zorder = 100 # Highest z-order
            
            # --- Wavelength (nm) plotting ---
            ...
            # --- NEW: Save data to plot on individual plots later ---
    ...
    for filepath in raw_files:
        ...
        try:
            ...
            # This is the raw convoluted spectrum
            ...
            # This is the (normalized * scaled) spectrum
            scaled_spectrum = normalized_total_spectrum * scale_factor
            
            # --- MODIFIED: Add to sum based on user's scaling choice ---
            if generate_total_computed:
                if scale_choice_raw:
                    # User chose to scale, so sum the (normalized * scaled) spectra
                    total_computed_spectrum_sum += scaled_spectrum
                else:
                    # User did not choose to scale, so sum the raw convoluted spectra
                    # We use 'total_spectrum' which is the raw, un-normalized convoluted spectrum
            ...
            # --- Create INDIVIDUAL plots for ALL raw files ---
            ...
            # --- NEW: Plot experimental data on individual plots ---
            ...
            # --- MODIFIED: Update title and Y-label to reflect scaling ---
            title_eV = f"Deconvolution for {label_eV_scaled} (Energy)"
            title_nm = f"Deconvolution for {label_nm_scaled} (Wavelength)"

            ax_ind_eV.set_xlabel("Energy (eV)", fontsize=14)
            # Y-label is now always 'Normalized Intensity' for consistency
            ax_ind_eV.set_ylabel("Normalized Intensity", fontsize=14) 
            ax_ind_eV.set_title(title_eV, fontsize=16)
            ax_ind_eV.grid(True, linestyle=':', alpha=0.6)
            ax_ind_eV.set_ylim(bottom=0)
            ax_ind_eV.legend(loc='best', fontsize=10) # Add legend
    ...
    # 11. --- MODIFIED: Plot the "Total Computed" spectrum (if requested) ---
    if generate_total_computed:
        print("  > Plotting summed computed spectrum...")
        # Note: total_computed_spectrum_sum now contains either raw sum or scaled-norm sum
        if np.max(total_computed_spectrum_sum) > 0:
            
            # --- Renormalize the sum (WHATEVER it is) before final scaling ---
            max_val_total = np.max(total_computed_spectrum_sum)
            normalized_total_computed_spectrum = total_computed_spectrum_sum / max_val_total
            
            shift_eV_total = manual_shift_eV_comp
            ...
            # --- Apply manual scaling to the NEWLY NORMALIZED sum ---
            scale_factor_total = scaling_factors.get('Total Computed', 1.0)
            scaled_total_spectrum = normalized_total_computed_spectrum * scale_factor_total # Apply scaling
            ...
            # --- Plot on P4 (CombVib, CombRaw, Exp) ---
            if ax_P4_eV:
    ...
    for filepath in vib_files:
        ...
        try:
            ...
            # This is the raw interpolated spectrum
            ...
            if generate_total_vibronic:
                if scale_choice_vib:
                    # User chose to scale, so sum the (normalized * scaled) spectra
                    total_vibronic_spectrum_sum += scaled_intensities
                else:
                    # User did not choose to scale, so sum the raw interpolated spectra
                    # We use 'intensities_interp' which is the raw, un-normalized spectrum
            ...
            # --- Plot on P3 (IndVib, CombRaw, Exp) ---
            if axes_P3[0]: # Check if plot exists
    ...
    # 11a. --- MODIFIED: Plot the "Total Vibronic" spectrum (if requested) ---
    if generate_total_vibronic and total_vibronic_spectrum_sum is not None:
        print("  > Plotting summed vibronic spectrum...")
        # Note: total_vibronic_spectrum_sum now contains either raw sum or scaled-norm sum
        if np.max(total_vibronic_spectrum_sum) > 0:
            
            # --- Renormalize the sum (WHATEVER it is) before final scaling ---
            max_val_total_vib = np.max(total_vibronic_spectrum_sum)
            normalized_total_vib_spectrum = total_vibronic_spectrum_sum / max_val_total_vib
            
            shift_eV_vib_total = manual_shift_eV_vib
            ...
            scaled_total_vib_spectrum = normalized_total_vib_spectrum * scale_factor_total_vib
    ...
    # 13. Finalize and save ALL plot sets
